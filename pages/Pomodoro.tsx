...
const Pomodoro: React.FC = () => {
  ...
  return (
    <div className="max-w-[1300px] mx-auto px-6 py-12 animate-in fade-in duration-500">
      <div className="text-center mb-20">
        ...
        <h1 className="text-6xl font-black text-slate-900 tracking-tighter mb-4">Focus Flow</h1>
        <p className="text-slate-500 font-medium">Precision study sessions for the elite mindset.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
        <div className="lg:col-span-7 flex flex-col items-center">
          ...
          <div className="flex gap-6 mt-20">
            <button onClick={toggleTimer} className="px-16 py-6 rounded-[2.5rem] font-black text-xl bg-indigo-600 text-white shadow-2xl shadow-indigo-200 hover:scale-105 active:scale-95 transition-all">
               {isActive ? 'Pause Flow' : 'Start Flow'}
            </button>
            <button onClick={resetTimer} className="w-20 h-20 bg-white border border-slate-100 rounded-full flex items-center justify-center text-slate-300 hover:text-indigo-600 hover:shadow-lg transition-all">
        ...
        <div className="lg:col-span-5 space-y-8">
             ...
             <div className="grid grid-cols-1 gap-3">
                {(Object.keys(MODES) as TimerMode[]).map(m => (
                  <button 
                    ...
                    className={`w-full p-6 rounded-2xl flex items-center justify-between transition-all border-2 ${
                      mode === m ? 'border-indigo-600 bg-indigo-50 shadow-sm' : 'border-slate-50 hover:border-slate-200 bg-slate-50/50'
                    }`}
                  >
                    <div className="flex items-center gap-4">
                       <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${mode === m ? 'bg-white text-indigo-600 shadow-sm' : 'bg-white text-slate-300'} `}>
                          <i className={`fas ${MODES[m].icon}`}></i>
                       </div>
                       <span className={`font-black ${mode === m ? 'text-indigo-600' : 'text-slate-600'}`}>{MODES[m].label}</span>
                    </div>
                    {mode === m && <i className="fas fa-check-circle text-indigo-600"></i>}
                  </button>
                ))}
             </div>
           </div>
          ...
          <div className="bg-slate-900 rounded-[3rem] p-10 text-white shadow-xl">
            <h3 className="text-[10px] font-black text-indigo-400 mb-10 uppercase tracking-widest">Today's Progress</h3>
            <div className="flex justify-between items-baseline mb-6">
              <p className="text-3xl font-black">{completedSessions} <span className="text-lg opacity-40">Sessions</span></p>
              <p className="text-xs font-bold text-slate-400">Goal: 8</p>
            </div>
            <div className="flex gap-2.5 h-3">
              {[...Array(8)].map((_, i) => (
                <div key={i} className={`flex-grow rounded-full transition-all duration-700 ${i < completedSessions ? 'bg-indigo-500 shadow-lg shadow-indigo-500/20' : 'bg-white/10'}`}></div>
              ))}
            </div>
          </div>
        </div>
      ...
      {showLogModal && (
        <div className="fixed inset-0 z-[60] bg-slate-900/60 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[3.5rem] p-12 max-w-lg w-full shadow-2xl border border-white/20 animate-in zoom-in-95 duration-500">
             <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-3xl flex items-center justify-center mx-auto mb-8 text-3xl shadow-inner">
             ...
             <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={logSession} className="flex-[2] py-5 bg-slate-900 text-white font-black rounded-[1.5rem] uppercase tracking-widest text-[10px] hover:bg-indigo-600 transition-all shadow-xl">Post to Feed</button>
                </div>
             </div>
          </div>
        </div>
