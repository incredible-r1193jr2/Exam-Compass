...
const Community: React.FC = () => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-8 flex flex-col lg:grid lg:grid-cols-12 gap-8 h-[calc(100vh-100px)]">
      <div className="lg:col-span-8 flex flex-col gap-6 overflow-y-auto minimal-scrollbar">
        ...
        {peerPosts.map((post) => (
          <div key={post.id} className="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-sm group hover:shadow-md transition-all">
      ...
      <div className="lg:col-span-4 flex flex-col h-full bg-white rounded-[3rem] border border-slate-100 overflow-hidden shadow-inner">
        ...
        <div className="flex-grow flex flex-col min-h-0 bg-white">
          <div className="flex-grow overflow-y-auto p-8 space-y-6 minimal-scrollbar">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'}`}>
                <div className={`flex items-end gap-3 max-w-[90%] ${msg.isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                  <img src={msg.avatar} alt={msg.user} className="w-8 h-8 rounded-xl bg-slate-50 border border-slate-100 shadow-sm" />
                  <div className={`px-5 py-3.5 rounded-2xl text-sm font-medium shadow-sm leading-relaxed ${msg.isMe ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-50 text-slate-700 border border-slate-100 rounded-tl-none'}`}>
                    {msg.text}
                  </div>
                </div>
          ...
          <div className="p-8 bg-white border-t border-slate-100">
            <div className="relative">
              ...
              />
              <button onClick={handleSendMessage} className="absolute right-3 top-1/2 -translate-y-1/2 w-12 h-12 bg-indigo-600 rounded-xl text-white flex items-center justify-center hover:bg-slate-900 transition-colors shadow-lg shadow-indigo-100">
